...
public class MainUI extends Settings implements PropertyChangeListener {
    ...
    @FXML
    private Pane stundenPlanAnsicht, modulAnsicht, topBar, settingsInputPain, settingsPane, editPaneBackground, editModulePane, errorPaneBackground, errorModulePane;

    @FXML
    private BorderPane popUpBackground;

    @FXML
    private Label titel, userStudienplanLabel, errorMsgLabel;

    @FXML
    private Button buttonTest, moduleAddButton, semesterAddButton, semesterRemoveButton, settingsButton, settingSaveButton, print, editModuleSaveButton, errorOkayButton, durchschnittsNote, saveStudienplan;

    @FXML
    private ImageView moduleAddIcon, semesterAddIcon, saveIcon;

    @FXML
    private ListView semesterListView;

    @FXML
    private ListHBox listHBox;

    @FXML
    private VBox studViewVBox;

    private boolean newUser;

    /**
     * Der Constructor der MainUI.
     * @param settings Die Global Settings werden per Dependency Injection übergeben.
     * @param newUser Boolean wert um zu erkennen, ob es sich um einen neuen User handelt.
     ...
     *  → Die listHBox besteht aus einer Pane die, die Semesterzahl anzeigt und einer ListView, in welcher die Module angezeigt werden.
    ...
    public void addSemester(ListHBox listHBox) {
        ...
        listHBox.setOnDragDropped(new EventHandler<DragEvent>() {
            ...
            public void handle(DragEvent event) {
                ...
                try {
                    Modul modulToAdd = settings.modulListeDomain.getModulFromList(Integer.parseInt(dragboard.getString()));
                    try {
        ...
        listHBox.setOnDragOver((dragEvent) -> {
            if(dragEvent.getGestureSource() != listHBox && dragEvent.getDragboard().hasContent(ModulBox.modulBoxFormat) || dragEvent.getDragboard().hasString()) {
    ...
    /*
    * JavaFX Initialisierung, hier werden Standardeinstellungen gesetzt und auf ActionEvents gehört.
    */
    @FXML
        ...
        */
        ...
        print.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                 ...
                 * Wir benutzen hier die pdfbox library, sowie ein Element aus der JavaFX Swing library.
                 */

                // Erstellen von einem temp. PNG der stundenPlanAnsicht (Pane)
                WritableImage nodeshot = stundenPlanAnsicht.snapshot(new SnapshotParameters(), null);
                ...
                try {
                    ...
                    //Damit der ganze Studienplan als PDF gedruckt werden kann, müssen wir das Layout kurz größer machen und danach wieder zurücksetzen.
                    ...
                    File file2 = fileChooser.showSaveDialog(((Node) event.getSource()).getScene().getWindow());
        ...
        for(int i = 0; i < stundenplanVerwaltung.getStundenplan().getMaxSemester(); i++) {
        ...
        semesterAddButton.setOnMouseClicked(event -> {
            stundenplanVerwaltung.addSemester();
        });

        semesterRemoveButton.setOnMouseClicked(event -> {
            stundenplanVerwaltung.removeSemester();
        });
        ...
        settingSaveButton.setOnMouseClicked(event -> {
            settings.setSTUD_NAME(((TextField) settingsBox.getChildren().get(1)).getText());
            settings.setMAX_CP(Integer.parseInt(((TextField) settingsBox.getChildren().get(3)).getText()));
            popUpBackground.setVisible(false);
            popUpBackground.setDisable(true);
            settingsPane.setVisible(false);
            settingsPane.setDisable(true);
        ...
        try {
            FXMLLoader modulListLoader = new FXMLLoader(App.class.getResource("modulList.fxml"));
            ModulListeUI modulListController = new ModulListeUI(settings.modulListeDomain);
        ...
        for(int i = 1; i <= stPlan.getMaxSemester(); i++) {
            ...
            for(Modul m: semMods) {
                ...
                ListHBox semesterListe = (ListHBox) semesterListView.getItems().get(semesterToAddTo);
                semesterListe.addModuleToView(m.getModulNummer());

            }
        }
        String durchString = String.valueOf(stundenplanVerwaltung.getDurchschnitt());
    ...
    public void propertyChange(PropertyChangeEvent event) {
        if (event.getPropertyName().equals(Settings.STUD_NAME_CHANGE)) {
            ...
            userStudienplanLabel.setText(settings.getSTUD_NAME() + "'s Studienplan");
        } else if(event.getPropertyName().equals((Stundenplan.ADD_SEMESTER_EVENT))) {
            addSemester(new ListHBox(semesterListView.getItems().size() + 1, settings.modulListeDomain));
            saveIcon.setImage(new Image("libs/images/saveIcon.png"));
            semesterListView.scrollTo(0);
        } else if(event.getPropertyName().equals((Stundenplan.REMOVE_SEMESTER_EVENT))) {
            saveIcon.setImage(new Image("libs/images/saveIcon.png"));
            semesterListView.getItems().remove(0);
            semesterListView.scrollTo(0);
        } else if(event.getPropertyName().equals((Stundenplan.REMOVE_MODULE_EVENT))) {
            saveIcon.setImage(new Image("libs/images/saveIcon.png"));
            ...
            int semesterToAddTo = semesterListView.getItems().size() - ((int) event.getOldValue());
            ListHBox semesterListe = (ListHBox) semesterListView.getItems().get(semesterToAddTo);
            semesterListe.addModuleToView(modulToAdd.getModulNummer());
            String durchString = String.valueOf(stundenplanVerwaltung.getDurchschnitt());
            durchschnittsNote.setText(durchString);
        } else if(event.getPropertyName().equals(ModulListeUI.EDIT_MODULE_ACTUATION)) {
            saveIcon.setImage(new Image("libs/images/saveIcon.png"));
            editModuleBox = new EditModuleBox((int) event.getNewValue(), settings.modulListeDomain);
        ...
        } else if(event.getPropertyName().equals(ModulListeDomain.NOTE_CHANGED_EVENT)) {
            saveIcon.setImage(new Image("libs/images/saveIcon.png"));
            String durchString = String.valueOf(stundenplanVerwaltung.getDurchschnitt());
            durchschnittsNote.setText(durchString);
